...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing roles", async() => {
        ...
        let isFarmer = await supplyChain.isFarmer(accounts[1]);
        let isDistributor = await supplyChain.isDistributor(accounts[2]);
        let isRetailer = await supplyChain.isRetailer(accounts[3]);
        let isConsumer = await supplyChain.isConsumer(accounts[4]);

        assert.equal(isFarmer, true, 'Error: Not a farmer')
        assert.equal(isDistributor, true, 'Error: Not a distributor')
        assert.equal(isRetailer, true, 'Error: Not a retailer')
        assert.equal(isConsumer, true, 'Error: Not a consumer')
    })
    ...
    it("Testing smart contract function shipItem() that allows a distributor to ship coffee", async() => {
        ...
        // Watch the emitted event Processed()
        var event = await supplyChain.Shipped((err, res) => {
            eventEmitted = true
        });
